...
uniform vec3 limitsMin;
uniform vec3 limitsMax;
...
void main()
{
    ...
    vec4 finalColor = (texelColor*((vec4(1.,1.,1.,1.) + vec4(specular,1))*vec4(lightDot, 1.0)));
    vec4 trueAmbient = vec4(0.1,0.1,0.1,1.0);
    finalColor += texelColor*(trueAmbient/10.0);
    ...
    if (any(lessThan(fragPosition.xyz, limitsMin)) || any(greaterThan(fragPosition.xyz, limitsMax)))
        discard;
}